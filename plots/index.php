...
class DirectoryListing {
	...
	*/
	...
	public $pageTitle = "Plots Directory (List view)";

	// The URL of this script. Optionally set if your server is unable to detect the paths of files
	public $includeUrl = 'https://etpwww.etp.kit.edu/~abal/plots/index.php';
...
$galleryUrl = htmlspecialchars(str_replace('/?', '/view.php?', str_replace('index', 'view', "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]")));
?>
<html>
<head>
	...
	<style>
		...
		.dir-icon { opacity: 0.55; font-size: 1.1rem; }
		.dir-name { font-weight: 500; font-size: 0.9rem; word-break: break-all; }
		.form-section {
			background: #f8f9fa;
			border: 1px solid #dee2e6;
			border-radius: 6px;
		...
		.thumb-wrap:hover .zoom-overlay { opacity: 1; background: rgba(0,0,0,0.28); pointer-events: auto; }
		.zoom-icon {
		...
		.lightbox-viewport {
			...
			min-height: 340px; max-height: 75vh;
			position: relative; padding: 0 !important; cursor: default;
		}
		#lightbox-canvas {
			display: flex; align-items: center; justify-content: center;
...
<body>
	...
	<header class="site-header" id="site-header">
		...
		<div class="d-flex align-items-center gap-2 flex-shrink-0">
			<div class="btn-group view-toggle-group" role="group" aria-label="View mode">
				...
				<a href="<?php echo $galleryUrl; ?>" class="btn btn-outline-primary">
			...
			<div class="col-12">
				...
				<?php foreach ($data['directoryTree'] as $url => $name): ?>
					...
					<li class="breadcrumb-item">
						<a href="?dir=<?php echo htmlspecialchars($url); ?>"><?php echo htmlspecialchars($name); ?></a>
					</li>
					<?php endif; ?>
				<?php endforeach; ?>
				</ol>
		...
		<div class="row mb-3">
			<div class="col-12">
				...
				<?php foreach ($data['directories'] as $directory): ?>
					<div class="col">
						<div class="position-relative">
							<a href="<?php echo $directory['url']; ?>" class="dir-card">
								<span class="dir-icon">&#128193;</span>
								<span class="dir-name"><?php echo htmlspecialchars($directory['name']); ?></span>
							</a>
							<?php if ($listing->enableDirectoryDeletion): ?>
							<a href="<?php echo $directory['url']; ?>&delete=true" class="btn btn-danger btn-sm position-absolute top-0 end-0" onclick="return confirm('Are you sure?')" style="font-size:0.7rem;padding:2px 6px;">&#x2715;</a>
							<?php endif; ?>
						</div>
					</div>
				<?php endforeach; ?>
				...
				<form action="" method="post" class="d-flex align-items-center gap-2 mt-2">
					<label class="mb-0 text-muted small">New directory:</label>
					<input type="text" name="directory" id="directory" class="form-control form-control-sm" style="width:auto;">
				...
				<table class="table table-hover table-sm align-middle">
					<thead class="table-light">
						<tr>
							...
							<th><a href="<?php echo $listing->sortUrl('name'); ?>">Image <span class="<?php echo $listing->sortClass('name'); ?>"></span></a></th>
							<th>Formats</th>
							<th class="text-end d-none d-md-table-cell"><a href="<?php echo $listing->sortUrl('modified'); ?>">Last Modified <span class="<?php echo $listing->sortClass('modified'); ?>"></span></a></th>
						</tr>
					</thead>
					<tbody>
					<?php foreach ($data['files'] as $file): ?>
						...
						<tr data-ext="png" class="lightbox-trigger"
							...
							<td>
								<div class="thumb-wrap">
									<img class="thumb-preview" src="<?php echo htmlspecialchars($file['url']); ?>" alt="<?php echo htmlspecialchars($file['name']); ?>">
							...
							<td>
								<a href="<?php echo htmlspecialchars($file['url']); ?>" class="lightbox-name-link">
									<?php echo htmlspecialchars($plotName); ?>
								</a>
								<button class="copy-btn" data-url="<?php echo htmlspecialchars($file['url']); ?>" title="Copy image URL">
									<i class="bi bi-clipboard"></i>
								</button>
							</td>
							<td>
								<a href="<?php echo $file['url']; ?>" target="<?php echo $file['target']; ?>" class="badge fmt-png text-decoration-none">.png</a>
								...
								<a href="<?php echo str_replace('png','C',$file['url']); ?>" target="<?php echo $file['target']; ?>" class="badge fmt-C text-decoration-none">.C</a>
								<?php endif; ?>
							</td>
							<td class="text-end d-none d-md-table-cell text-muted"><?php echo date('M jS Y \a\t g:ia', $file['modified']); ?></td>
						</tr>
						<?php endif; ?>
					<?php endforeach; ?>
					</tbody>
		...
		<div class="row mt-2">
			<div class="col-12">
				<table class="table table-hover table-sm align-middle">
					<thead class="table-light">
						<tr>
							<th><a href="<?php echo $listing->sortUrl('name'); ?>">File <span class="<?php echo $listing->sortClass('name'); ?>"></span></a></th>
							<th class="text-end d-none d-sm-table-cell"><a href="<?php echo $listing->sortUrl('size'); ?>">Size <span class="<?php echo $listing->sortClass('size'); ?>"></span></a></th>
							<th class="text-end d-none d-md-table-cell"><a href="<?php echo $listing->sortUrl('modified'); ?>">Last Modified <span class="<?php echo $listing->sortClass('modified'); ?>"></span></a></th>
						</tr>
					</thead>
					<tbody>
					<?php foreach ($data['files'] as $file): ?>
						<?php if ($file['mime'] != 'image/png'): ?>
						<tr data-ext="<?php echo htmlspecialchars($file['extension']); ?>">
							<td>
								<?php if ($file['extension'] === 'pdf'): ?>
								<a href="<?php echo htmlspecialchars($file['url']); ?>" class="pdf pdf-preview-link" data-pdf-name="<?php echo htmlspecialchars($file['name']); ?>">
									<?php echo htmlspecialchars($file['name']); ?>
								</a>
								<?php else: ?>
								<a href="<?php echo htmlspecialchars($file['url']); ?>" target="<?php echo $file['target']; ?>" class="<?php echo htmlspecialchars($file['extension']); ?>">
									<?php echo htmlspecialchars($file['name']); ?>
								</a>
								<?php endif; ?>
								<?php if ($listing->enableFileDeletion): ?>
								<a href="?deleteFile=<?php echo urlencode($file['relativePath']); ?>" class="float-end btn btn-danger btn-sm" onclick="return confirm('Are you sure?')">Delete</a>
								<?php endif; ?>
							</td>
							<td class="text-end d-none d-sm-table-cell text-muted"><?php echo $file['size']; ?></td>
							<td class="text-end d-none d-md-table-cell text-muted"><?php echo date('M jS Y \a\t g:ia', $file['modified']); ?></td>
						</tr>
						<?php endif; ?>
					<?php endforeach; ?>
	...
	<!-- PDF Preview Modal -->
	<div class="modal fade" id="pdf-modal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-xl" style="max-height:92vh;">
			<div class="modal-content" style="height:85vh;display:flex;flex-direction:column;">
				<div class="modal-header py-2">
					<h6 class="modal-title" id="pdf-modal-title" style="font-family:'IBM Plex Mono',monospace;font-size:0.82rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:80%;"></h6>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body p-0" style="flex:1;min-height:0;overflow:hidden;">
					<iframe id="pdf-frame" src="" style="width:100%;height:100%;border:none;display:block;"></iframe>
				</div>
				<div class="modal-footer py-2 justify-content-end">
					<a id="pdf-open" href="#" target="_blank" class="btn btn-sm btn-outline-primary">
	...
	<script>
		(function() {
			...
			document.querySelectorAll('.lightbox-trigger').forEach(function(row, i) {
				items.push({
					url:    row.dataset.imgUrl,
					name:   row.dataset.plotName,
					pdfUrl: row.dataset.pdfUrl || ''
				});
				var thumbWrap = row.querySelector('.thumb-wrap');
				var nameLink  = row.querySelector('.lightbox-name-link');
				if (thumbWrap) thumbWrap.addEventListener('click', function() { showLightbox(i); });
				if (nameLink)  nameLink.addEventListener('click',  function(e) { e.preventDefault(); showLightbox(i); });
			});
			...
			lbViewport.addEventListener('mousedown', function(e) {
				...
				isPanning = true;
				panStartX = e.clientX - zX; panStartY = e.clientY - zY;
				lbViewport.classList.add('is-panning');
				e.preventDefault();
			});
			document.addEventListener('mousemove', function(e) {
				if (!isPanning) return;
				zX = e.clientX - panStartX; zY = e.clientY - panStartY;
				applyZoom();
			});
			...
			lbViewport.addEventListener('touchstart', function(e) {
				if (e.touches.length === 2) {
					...
					touchPanning = false; e.preventDefault();
				} else if (e.touches.length === 1 && zScale > 1) {
					touchPanning = true;
					panStartX = e.touches[0].clientX - zX; panStartY = e.touches[0].clientY - zY;
					e.preventDefault();
				}
			}, { passive: false });
			lbViewport.addEventListener('touchmove', function(e) {
				if (e.touches.length === 2 && lastTouchDist !== null) {
					...
					var rect = lbViewport.getBoundingClientRect();
					zoomToward((e.touches[0].clientX + e.touches[1].clientX) / 2 - rect.left,
					           (e.touches[0].clientY + e.touches[1].clientY) / 2 - rect.top, dist / lastTouchDist);
					lastTouchDist = dist; e.preventDefault();
				} else if (e.touches.length === 1 && touchPanning) {
					zX = e.touches[0].clientX - panStartX; zY = e.touches[0].clientY - panStartY;
			...
			lbViewport.addEventListener('dblclick', function(e) {
				var rect = lbViewport.getBoundingClientRect();
				if (zScale === 1) zoomToward(e.clientX - rect.left, e.clientY - rect.top, 3);
				else resetZoom();
			});

			document.getElementById('zoom-in').addEventListener('click', function() { zoomToward(lbViewport.offsetWidth / 2, lbViewport.offsetHeight / 2, 1.5); });
			document.getElementById('zoom-out').addEventListener('click', function() { zoomToward(lbViewport.offsetWidth / 2, lbViewport.offsetHeight / 2, 1 / 1.5); });
			...
			document.getElementById('lightbox-copy').addEventListener('click', function() {
				...
				navigator.clipboard.writeText(url).then(function() {
					...
					btn.textContent = ''; btn.appendChild(ic); btn.appendChild(document.createTextNode('Copied!'));
					setTimeout(function() {
						ic.className = 'bi bi-clipboard me-1';
						btn.textContent = ''; btn.appendChild(ic); btn.appendChild(document.createTextNode('Copy link'));
					}, 2000);
				});
			});
		})();
	...
	<script>
		(function() {
			var pdfModal = null;
			document.querySelectorAll('.fmt-pdf, .pdf-preview-link').forEach(function(link) {
				link.addEventListener('click', function(e) {
					e.preventDefault();
					e.stopPropagation();
					var url  = this.href;
					var name = this.dataset.pdfName || (function() {
						var row = link.closest('tr[data-plot-name]');
						return row ? row.dataset.plotName + '.pdf' : url;
					})();
					document.getElementById('pdf-frame').src        = url;
					document.getElementById('pdf-modal-title').textContent = name;
					document.getElementById('pdf-open').href        = url;
					if (!pdfModal) pdfModal = new bootstrap.Modal(document.getElementById('pdf-modal'));
					pdfModal.show();
				});
			});
			document.getElementById('pdf-modal').addEventListener('hide.bs.modal', function() {
